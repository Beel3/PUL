...
function saveFeed($name, $email, $message)
{
	global $cont;
	$sql=$cont->prepare("INSERT INTO feedback (name, email, message) values ('" . $name . "','" . $email . "','" . $message . "')");
	$result= $sql->execute(array($name, $email, $message));	
}
...
{
	global $cont;
	$result = $cont->query("SELECT * FROM `feedback` ");
	return $result;
}
?>